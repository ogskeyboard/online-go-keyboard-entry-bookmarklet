		...
		$("#bookmarklet-move").on('keyup', function(ev) {
			if (ev.which==13||ev.which==32) {
				...
				if (mv.length>=2 && mv.length<=3) {
					var x=mv.charAt(0);
					var y=mv.substring(1);
					y=global_goban.height - parseInt(y);
					if (x.charCodeAt(0) > 104)
						x = String.fromCharCode(x.charCodeAt(0)-1);
					...
					if (y>=0 && y<26 && x.charCodeAt(0)>=97 && x.charCodeAt(0)<=115) {
						...
						if (y>=0 && y<global_goban.height && x>=0 && x<global_goban.width) {
							if (global_goban.__last_pt.valid) {
								var b = global_goban.last_hover_square; 
								global_goban.last_hover_square = null; 
								global_goban.drawSquare(b.x, b.y);
							}
							global_goban.__last_pt = { valid:true, i: x, j: y };
							global_goban.last_hover_square = {"x": x,"y": y};
							global_goban.drawSquare(x, y);
						}
					}
				}
			}
